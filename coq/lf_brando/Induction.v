...
Proof.
  intros n. induction n as [| n IHn].
    (* double 0 = 0 + 0 *)
  - unfold double. unfold plus. reflexivity. 
    (* double (S n) = S n + S n *)
  - simpl. rewrite -> IHn. rewrite -> plus_n_Sm. reflexivity.
Qed.
(** [] *)
...
(* FILL IN HERE *)
Destruct shows that a statement is true for each constructor,
while induction shows that a bigger object has a certain property
assuming that the smaller objects build using the constructors have that property.
i.e. P(n)=>P(n+1) here we used the +1=S constructor which shows P(n+1) is true.
In fact its clearer if we write it as follows:
P(Sn) => P(n') where n' is the "bigger" object.
Advantage over induction is that we have the IH being true over the constructors.
In this case only successor is the constructor and thus we could have only built n'
via it. We can use P(Sn) to show P(n') on the bigger one. 
We consider P(n') as check if its true assuming P(Sn).
While for destruct we only have P(n) and we just check each case seperately.
We don't really consider if it was built from other stuff, just if P(Sn) is true for each case separetaly.
*)
...
Proof.
  intros n m p.
  induction p as [| p IH].
  - replace (m + 0) with (m). replace (n+0) with (n). rewrite -> plus_comm. reflexivity.
...
Lemma plus_O_nop2: forall n: nat,
  n + O = n.
Proof.
  induction n.
  (* n = 0 *)
