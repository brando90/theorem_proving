...
Example app_eqs: app [1;2;3] [4] = app' [1;2;3] [4].
Proof.
...
Proof.
  unfold nonzeros. reflexivity.
Qed.

(* oddmembers keeps all the odd members and removes the even ones *)
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | cons h l => match (evenb h) with
                | false => cons h (oddmembers l)
                | true => oddmembers l
                end
  end.
...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  ...
  | nil, _ => l2
  | _, nil => l1
  | h1 :: end1, h2 :: end2 => h1 :: h2 :: (alternate end1 end2)
  end.
...
Fixpoint count2 (v:nat) (s:bag) : nat :=
  match s with
  | nil => 0
  | h :: tl => if (beq_nat v h) then 1 + (count v tl) else count v tl
  end.
...
Proof. simpl. reflexivity. Qed.
Example test_count3:              count 6 [1;2;3;1;4;1] = count2 6 [1;2;3;1;4;1].
Proof. simpl. reflexivity. Qed.
...
Fixpoint contains (s:bag) (v:nat) : bool :=
  match s with
  | nil => false
  | h :: tl => if beq_nat v h then true else contains tl v
  end.

Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | h :: tl => if contains s2 h then subset tl (remove_one h s2) else false
  end.                
...
Proof.
  intros l1 l2. induction l1 as [| n l IHn].
  - simpl. reflexivity.
...
Compute record (Id 2) 3 empty.
Compute record (Id 2) 3 (record (Id 2) 3 empty).
(*
     = record (Id 2) 3 (record (Id 2) 3 empty)
     : partial_map
*)
...
Compute update (record (Id 2) 3 empty) (Id 2) (4).
(* 
     = record (Id 2) 4 (record (Id 2) 3 empty)
     : partial_map
*)
