...
Add LoadPath "/Users/brandomiranda/home_simulation_research/auto_maths_papers/coq/lf_brando".
...
Check nil2. (* nil : forall X : Type, list X *)
Check nil2 nat. (* nil nat: list nat *)

Check cons2. (* cons : forall X : Type, X -> list X -> list X *)
Check cons2 nat. (* cons2 nat : nat -> list2 nat -> list2 nat *)
Check cons2 nat 3. (* cons2 nat 3 : list2 nat -> list2 nat *)
Check (cons2 nat 3 (nil2 nat)). (* cons2 nat 3 (nil2 nat) : list2 nat *)

Compute (cons2 nat 2) (nil2 nat).
...
(*
I think I prefer the syntax above. It's more similar to how types for functions
work. If we have f:A->B we expect that the input to f to be of type B, that is
what the syntax for cons looks like (although, its annoying cuz (x:Type) seems 
a better syntax cuz we can statically in the definition of the type
see the name for the spcific instance being passed/used.
I guess we have two different syntaxes. One for the names of the functions within
real Coq and the ones that are constructors (functions).
*)
...
Check nil.
(* sort of a function/constructor that for any Type X it constructs the object
nil of type List X. *)
Check nil nat.
(* nil corresponding to nats *)
Check nil bool.
...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      - [d (b a 5)], no because d expects a type X as input
      - [d mumble (b a 5)], yes this is well-typed
      - [d bool (b a 5)], yes wekk typed, note that d NEEDS a type X cuz all grumble's need X:Type
      - [e bool true], yes its a grumble bool
      - [e mumble (b c 0)], Yes, since next argument matches type mumble
      - [e bool (b c 0)], No, since next argument does NOT match type mumble
      - [c], yes just mumble
*)
(* FILL IN HERE *)
Compute d nat (b a 5).
...
Compute d.
(*
     = fun (X : Type) (x : mumble) => d X x
     : forall X : Type, mumble -> grumble X
*)
Compute d bool.
(*
     = fun x : mumble => d bool x
     : mumble -> grumble bool
 *)
...
Fixpoint repeat3 (X : Type) (x : X) (count : nat) : list X :=
  match count with
  | 0 => nil X
  | S count' => cons X x (repeat3 X x count')
  end.
...
Inductive single' {X : Type} : Type :=
| sing' : X -> single'.

Compute sing'.
Fail Compute sing' nat 1.
Compute sing' 1.
...
(* the implicit argument goes to the constructors! *)
Arguments sing {X} _.
...
Compute pair.
Compute pair nat.
Compute pair nat nat.
Compute (pair nat nat) 2.
Compute pair nat nat 2 3.
